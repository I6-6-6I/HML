...
HEADER_MATH_NAMESPACE
{
	...
	#pragma region CONSTRUCTORS
		...
		{
			this->values[0] = vector<C, T>(scalar, static_cast<T>(0));
			this->values[1] = vector<C, T>(static_cast<T>(0), scalar);
			this->values[2] = vector<C, T>(static_cast<T>(0), static_cast<T>(0));
			this->values[3] = vector<C, T>(static_cast<T>(0), static_cast<T>(0));
		}
		constexpr matrix(T const& x1, T const& y1, T const& x2, T const& y2, T const& x3, T const& y3, T const& x4, T const& y4)
		{
			this->values[0].x = x1; this->values[0].y = y1;
			this->values[1].x = x2; this->values[1].y = y2;
			this->values[2].x = x3; this->values[2].y = y3;
			this->values[3].x = x4; this->values[3].y = y4;
		}
	...
	#pragma region CONVERSIONS
		...
		constexpr matrix(U const& x1, V const& y1, M const& x2, N const& y2, P const& x3, D const& y3, Z const& x4, Y const& y4)
		{
			this->values[0].x = x1; this->valuesp[0].y = y1;
			this->values[1].x = x2; this->valuesp[1].y = y2;
			this->values[2].x = x3; this->valuesp[2].y = y3;
			this->values[3].x = x4; this->valuesp[3].y = y4;
		}
...
#pragma region BINARY_ARITHMETHIC_OPERATORS
	...
	template<typename T>
	matrix<2, 2, T> operator*(matrix<R, C, T> const& m1, matrix<2, 4, T> const& m2)
	{
		return matrix<2, 2, T>(
	...
	template<typename T>
	matrix<3, 2, T> operator*(matrix<R, C, T> const& m1, matrix<3, 4, T> const& m2)
	{
		return matrix<3, 2, T>(
			m1[0][0] * m2[0][0] + m1[1][0] * m2[0][1] + m1[2][0] * m2[0][2] + m1[3][0] * m2[0][3],
			m1[0][1] * m2[0][0] + m1[1][1] * m2[0][1] + m1[2][1] * m2[0][2] + m1[3][1] * m2[0][3],
